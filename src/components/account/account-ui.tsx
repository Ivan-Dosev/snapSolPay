...
export function AccountBalance({ address }: { address: PublicKey }) {
  return (
    <QueryClientProvider client={queryClient}>
      <AccountBalanceContent address={address} />
    </QueryClientProvider>
  )
}

function AccountBalanceContent({ address }: { address: PublicKey }) {
...
export function AccountBalanceCheck({ address }: { address: PublicKey }) {
  return (
    <QueryClientProvider client={queryClient}>
      <AccountBalanceCheckContent address={address} />
    </QueryClientProvider>
  )
}

function AccountBalanceCheckContent({ address }: { address: PublicKey }) {
...
export function AccountTokens({ address }: { address: PublicKey }) {
  return (
    <QueryClientProvider client={queryClient}>
      <AccountTokensContent address={address} />
    </QueryClientProvider>
  )
}

function AccountTokensContent({ address }: { address: PublicKey }) {
...
export function AccountTransactions({ address }: { address: PublicKey }) {
  return (
    <QueryClientProvider client={queryClient}>
      <AccountTransactionsContent address={address} />
    </QueryClientProvider>
  )
}

function AccountTransactionsContent({ address }: { address: PublicKey }) {
  const query = useGetSignatures({ address })
  const [showAll, setShowAll] = useState(false)
  const [showDashboard, setShowDashboard] = useState(false)

  // Toggle between table and dashboard views
  useEffect(() => {
    const dashboardElement = document.getElementById('transaction-dashboard')
    const placeholderElement = document.getElementById('transaction-dashboard-placeholder')
    
    if (dashboardElement && placeholderElement) {
      if (showDashboard) {
        // Move the dashboard into the placeholder
        dashboardElement.classList.remove('hidden')
        placeholderElement.appendChild(dashboardElement)
      } else {
        // Reset state - move dashboard back to its original position
        try {
          dashboardElement.classList.add('hidden')
          document.querySelector('.container')?.appendChild(dashboardElement)
        } catch (e) {
          // Handle case where DOM might have changed
          console.log('Error resetting dashboard position', e)
        }
      }
    }
  }, [showDashboard])
  ...
  return (
    <div className="space-y-2">
      <div className="flex justify-between">
        ...
        <div className="space-x-2">
          <button 
            className={`btn btn-sm ${showDashboard ? 'btn-outline' : 'btn-primary'}`}
          ...
          <button 
            className={`btn btn-sm ${showDashboard ? 'btn-primary' : 'btn-outline'}`}
      ...
      {query.isSuccess && (
        <div>
          {query.data.length === 0 ? (
            ...
            <div className="pt-2">
              {/* This will be imported and rendered in account-detail-feature.tsx */}
              <div id="transaction-dashboard-placeholder"></div>
            </div>
          ) : (
            <table className="table border-4 rounded-lg border-separate border-base-300">
              <thead>
...
function ModalAirdrop({ hide, show, address }: { hide: () => void; show: boolean; address: PublicKey }) {
  return (
    <QueryClientProvider client={queryClient}>
      <ModalAirdropContent hide={hide} show={show} address={address} />
    </QueryClientProvider>
  )
}

function ModalAirdropContent({ hide, show, address }: { hide: () => void; show: boolean; address: PublicKey }) {
  const mutation = useRequestAirdrop({ address })
...
function ModalSend({ hide, show, address }: { hide: () => void; show: boolean; address: PublicKey }) {
  return (
    <QueryClientProvider client={queryClient}>
      <ModalSendContent hide={hide} show={show} address={address} />
    </QueryClientProvider>
  )
}

function ModalSendContent({ hide, show, address }: { hide: () => void; show: boolean; address: PublicKey }) {
  const wallet = useWallet()
