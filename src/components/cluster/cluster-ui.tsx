...
export function ClusterUiSelect({ className }: { className?: string }) {
  ...
  return (
    <div className={`dropdown dropdown-end ${className || ''}`}>
      <label tabIndex={0} className="btn rounded-lg bg-primary text-white border-none h-10 min-h-0 min-w-[120px] flex items-center gap-2">
      ...
      <ul tabIndex={0} className="menu dropdown-content z-[1] p-2 shadow bg-base-100 rounded-box w-52 mt-4">
        {clusters.map((item) => (
          <li key={item.name}>
            <button
              className={`btn btn-sm ${item.active ? 'bg-primary text-white' : 'btn-ghost'}`}
